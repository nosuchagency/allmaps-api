...
class ContainersDeleteTest extends TestCase
{
    ...
    /** @test */
    public function an_authenticated_user_with_delete_permission_may_delete_specific_container()
    {
    ...
    /** @test */
    public function an_authenticated_user_with_delete_permission_may_delete_containers_in_bulk()
    {
        $this->signIn()->assignRole(
            $this->createRoleWithPermissions(['containers.delete'])
        );
        ...
        $this->postJson(route('containers.bulk-destroy'), ['items' => $containers])->assertStatus(200);
